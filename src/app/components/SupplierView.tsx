  ...
  if ('orders' in props) {
    ...
    return (
      <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
        <header className="text-white p-4 pb-4 rounded-b-2xl shadow-lg relative overflow-hidden" style={{ backgroundColor: '#FF6B00' }}>
    ...
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-gray-100 p-6 space-y-4 rounded-t-[2.5rem] shadow-2xl z-20">
        <div className="flex items-center justify-between px-2">
          <div>
            <p className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">Сумма заказа</p>
            <p className="text-3xl font-black text-gray-900 leading-none">
              {totalAmount.toLocaleString()} <span className="text-sm font-bold text-gray-400 uppercase ml-1">сум</span>
            </p>
          </div>
          <div className="text-right">
            <p className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">Заполнено</p>
            <p className="text-xl font-black text-orange-600 leading-none">{totalWithPrice} / {filteredProducts.length}</p>
          </div>
        </div>

        <button
          onClick={handleSend}
          disabled={totalWithPrice === 0}
          ...
          style={{ backgroundColor: '#FF6B00', boxShadow: '0 20px 25px -5px rgba(255, 107, 0, 0.3)' }}
        >
          <Send className="w-5 h-5" />
          Отправить финансисту
        </button>
      </div>
    </div>
  );
